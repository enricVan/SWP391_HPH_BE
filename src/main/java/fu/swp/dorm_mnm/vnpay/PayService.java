...
public class PayService {
    ...
    public String payWithVNPAY(PaymentDto paymentDto, HttpServletRequest request) throws UnsupportedEncodingException {
        Calendar cld = Calendar.getInstance(TimeZone.getTimeZone("Etc/GMT+7"));

        SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMddHHmmss");
        String vnp_CreateDate = formatter.format(cld.getTime());

        Long totalPriceByVnd = (long) (paymentDto.getAmount());

        if (paymentDto.getPaymentId() == null) {
            return "payment not exits";
        }
        paymentDto.vnp_TxnRef = Config.getRandomNumber(8);
        ...
        vnp_Params.put("vnp_OrderInfo", paymentDto.vnp_OrderInfo);
        vnp_Params.put("vnp_OrderType", paymentDto.vnp_OrderType);
        vnp_Params.put("vnp_ReturnUrl", VnPayConstant.vnp_ReturnUrl);
        vnp_Params.put("vnp_TxnRef", String.valueOf(paymentDto.getPaymentId()) + String.valueOf(paymentDto.vnp_TxnRef));
    ...
    public RedirectView handleTransaction() {
        ...
        return new RedirectView("http://localhost:5173/paid-success");
